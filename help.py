...
class Command:
    def __init__(
        self, group, command, discription, usage, name, output=None, aliases=None
    ):
        self.group = group
        self.command = command
...
class Cog(commands.Cog):
    ...
    async def help(self, ctx, group=None, subcommand=None):
        # sourcery no-metrics
        math = CommandGroup("**🤓 Math**", f"`{ctx.prefix}help math`", "math")
        add = Command(
            math,
            "Add",
        ...
        cc = CommandGroup(
            "**🤖 Custom Commands**", f"`{ctx.prefix}help cc`", "cc"
        )
        list = Command(
            cc,
            "List",
        ...
        fun = CommandGroup("**🎮 Fun**", f"`{ctx.prefix}help fun`", "fun")
        xo = Command(
            fun,
            "TicTacToe",
        ...
        misc = CommandGroup("**✨ Misc**", f"`{ctx.prefix}help misc`", "misc")
        coolbot = Command(
            misc,
            "Cool Bot 😎",
        ...
        leaderboard = Command(
            ...
            f"`{ctx.prefix}leaderboard`",
            "leaderboard",
            aliases=["`lb`", "`leaderboard`"],
        )

        groups = [math, cc, tier, fun, misc]
        group_names = [group.name for group in groups]

        math.commands = [add, subtract, mult, div, mean, median, mode]
        cc.commands = [enable, disable, list, ladd, toggle, remove]

...
class Cog(commands.Cog):
    ...
    async def help(self, ctx, group=None, subcommand=None):
        ...
        tier = CommandGroup('**🎖 Messaging Tiers**',
                            f'`{ctx.prefix}help tier`', 'tier')
        deduct = Command(tier, 'Deduct Point', 'Deducts messaging points from a user. (Admins Only)',
                         f'`{ctx.prefix}tier deduct |mention member|`', 'deduct')
        score = Command(tier, 'Messaging Score', 'Shows score and rank of a person or yourself in this server.',
                        f'`{ctx.prefix}tier score |mention member|`', 'score')
        exclude = Command(tier, 'Exclude Channel', 'Excludes said channel from adding up your scores. Useful to nerf spam channels etc.',
        ...
        if group == None:
            ...
            help = discord.Embed(title=command.command,
                                 color=PURPLE, description=command.discription)

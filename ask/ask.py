...
class askCog(commands.Cog):
    ...
    def get_data(self):
        ...
        with open('ask/ask.json', 'r') as f:
            data = json.load(f) # Converts the contents of the json file into a python dictionary and stores it in "data"
    ...
    async def ask(self, ctx, question=None): # "question" is set to None by default. It is to determine wether a question
        # asked or if it was invoked empty. 

        if question is None: # Runs everything under this if statement is True ie. if there is no question asked.
            embed = discord.Embed(title='Ask a question dude.', color=PURPLE) # Defines an embed because well its prettier than messages lol.
            await ctx.reply(embed=embed, mention_author=False) # Replies to the message.
        ...
        quote = '"'+random.choice(self.responses)+'"' # Same thing but adds quotes ("") to the random response.
        year = random.randint(1200, 2200) # Random year.
        font = random.choice(self.fonts) # Chooses random font.
        font = ImageFont.truetype(font, person.size) # Initalises font with image manipulation library.
        image = Image.open(person.file) # Loads image file to edit.
        drawing = ImageDraw.Draw(image) # Imagine it like setting up the image on a canvas ready to add some text

        drawing.text((person.x, person.y), quote, font=font, fill=person.color) # Draws the text according to the persons attributes (x, y and colour).
        image.save("ask/Tempquote.png") # Saves edited image in the folder "ask" (this folder) and in the name "Tempquote.png". 
        # NOTE: The "save" funtion overwrites it if a file already exists in the name. So we dont have to delete it after each time.

        output = discord.File('ask/Tempquote.png') # Initializes the file to discord for sending it in an embed.
        embed = discord.Embed(title=f'{person.name.upper()} HAS SPOKEN', description=' ', color=PURPLE) # NOTE: "upper" makes a string uppercase.
        embed.set_image(url='attachment://ask/Tempquote.png') # This is a little workaround for sending a local image because discord embeds usually only allow urls.
        embed.set_footer(text=f'~{person.name}, {year}') 

        await ctx.reply(file=output, embed=embed, mention_author=False) # Replies to the command with final embed.
